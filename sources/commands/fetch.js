...
module.exports = class FetchCommand extends BaseCommand {
  ...
  static usage = Command.Usage({
    ...
    `,
    examples: [
      [`yarn fetch --immutable`, `yarn fetch workspace my-package focus`],
    ],
  });
  ...
  async execute() {
    ...
    if (protectPackageJson) {
      this.context.stdout.write(
        "[YARN-FETCH] backup possible package.json file(s)\n"
      );
      ...
      packageJsonFiles = workspacePackages.map((packageVersion) => {
        const { resolution } = lockJson[packageVersion];
      ...
      packageJsonFiles.forEach((file) => {
        if (
          fs.existsSync(file) &&
          !fs.existsSync(`${file}.yarn-plugin-fetch-bak`)
        ) {
    ...
    try {
      ...
      if (protectPackageJson) {
        this.context.stdout.write(
          "[YARN-FETCH] restoring possible package.json file(s)\n"
        );
        packageJsonFiles.forEach((file) => {
